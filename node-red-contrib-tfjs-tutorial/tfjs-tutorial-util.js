...
const processInput = function (imageBuffer) {
  ...
  return [imageBuffer, tf.node.decodeImage(uint8array, 3).expandDims()];
};
const maxNumBoxes = 5;
...
const calculateNMS = function (outputBoxes, maxScores) {
  ...
  const boxes = tf.tensor2d(outputBoxes.dataSync(), [
    outputBoxes.shape[1],
    outputBoxes.shape[3],
  ]);
...
const createJSONresponse = function (
  boxes,
  scores,
  indexes,
  classes,
  ...
  for (let i = 0; i < count; i++) {
    ...
    const minY = bbox[0] * height - 50;
    const minX = bbox[1] * width - 50;
    const maxY = bbox[2] * height - 50;
    const maxX = bbox[3] * width - 50;
